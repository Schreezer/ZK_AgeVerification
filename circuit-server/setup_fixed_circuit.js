/**
 * Setup script for the fixed age verification circuit
...
// Constants
const CIRCUIT_PATH = path.join(__dirname, 'fixed_age_verification.circom');
const OUTPUT_DIR = path.join(__dirname, 'fixed_age_verification_js');
const PTAU_PATH = path.join(__dirname, 'pot14_final.ptau');
const ZKEY_PATH = path.join(__dirname, 'fixed_age_verification.zkey');
const VKEY_PATH = path.join(__dirname, 'fixed_age_verification_verification_key.json');

/**
 * Main function to set up the circuit
 */
async function main() {
    console.log('=== Setting up Fixed Age Verification Circuit ===');
    ...
    try {
        ...
        console.log('\n3. Generating zkey...');
        execSync(`snarkjs groth16 setup ${path.join(OUTPUT_DIR, 'fixed_age_verification.r1cs')} ${PTAU_PATH} ${ZKEY_PATH}`, { stdio: 'inherit' });
        ...
        console.log('You can now run the fixed age verification flow');
    } catch (error) {
